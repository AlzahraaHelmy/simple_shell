...
/**
 * get_histo_file - gets the history
 * @info: parameter
 *
...
/**
 * wrt_history - creates a file, or appends
 * @info: the parameter
 *
...
int rd_history(info_t *info)
{
	...
	struct stat st;
	char *buf = NULL, *filename = get_histo_file(info);
	...
	for (i = 0; i < fsize; i++)
		...
		{
			buf[i] = 0;
			build_histrooy_list(info, buf + last, linecount++);
			last = i + 1;
		}
	if (last != i)
		build_histrooy_list(info, buf + last, linecount++);
	...
	while (info->histcount-- >= HIST_MAX)
		delete_node_in_indx(&(info->history), 0);
	renum_history(info);
	return (info->histcount);
}

/**
 * build_histrooy_list - adds entry to a history ll
 * @info: Structure
 * @buf:the buffer
...
int build_histrooy_list(info_t *info, char *buf, int linecount)
{
	...
	if (info->history)
		node = info->history;
	add_nod_to_end(&node, buf, linecount);
...
/**
 * renum_history - re number the history linked list
