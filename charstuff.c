#include "shell.h"

/**
 * is_chain - test if
 * @info: the parameter
 * @buf: the char buffer
 * @p: address of current
 *
 * Return: 1 or 0
 */
int is_chain(info_t *info, char *buf, size_t *p)
{
	size_t j = *p;

	if (buf[j] == '|' && buf[j + 1] == '|')
	{
		buf[j] = 0;
		j++;
		info->cmd_buf_type = CMD_OR;
	}
	else if (buf[j] == '&' && buf[j + 1] == '&')
	{
		buf[j] = 0;
		j++;
		info->cmd_buf_type = CMD_AND;
	}
	else if (buf[j] == ';')
	{
		buf[j] = 0;
		info->cmd_buf_type = CMD_CHAIN;
	}
	else
		return (0);
	*p = j;
	return (1);
}

/**
 * check_ch - checks
 * @info: the parameter
 * @buf: the char
 * @p: address current position
 * @i: starting
 * @len: length of buf
 * Return: Void
 */
void check_ch(info_t *info, char *buf, size_t *p, size_t i, size_t len)
{
	size_t j = *p;

	if (info->cmd_buf_type == CMD_AND)
	{
		if (info->status)
		{
			buf[i] = 0;
			j = len;
		}
	}
	if (info->cmd_buf_type == CMD_OR)
	{
		if (!info->status)
		{
			buf[i] = 0;
			j = len;
		}
	}

	*p = j;
}

/**
 * rep_alias - replaces an aliases
 * @info: the parameter
 *
 * Return: 1 or 0
 */
int rep_alias(info_t *info)
{
	int i;
	list_t *node;
	char *p;

	for (i = 0; i < 10; i++)
	{
		node = node_starting_with(info->alias, info->argv[0], '=');
		if (!node)
			return (0);
		free(info->argv[0]);
		p = _stringchr(node->str, '=');
		if (!p)
			return (0);
		p = _strdup(p + 1);
		if (!p)
			return (0);
		info->argv[0] = p;
	}
	return (1);
}

/**
 * rep_vars - replaces vars in string
 * @info: the parameter struct
 * Return: 1 if replaced, 0 otherwise
 */
int rep_vars(info_t *info)
{
	int i = 0;
	list_t *node;

	for (i = 0; info->argv[i]; i++)
	{
		if (info->argv[i][0] != '$' || !info->argv[i][1])
			continue;

		if (!_strcmp(info->argv[i], "$?"))
		{
			rep_string(&(info->argv[i]),
				_strdup(conv_number(info->status, 10, 0)));
			continue;
		}
		if (!_strcmp(info->argv[i], "$$"))
		{
			rep_string(&(info->argv[i]),
				_strdup(conv_number(getpid(), 10, 0)));
			continue;
		}
		node = node_starting_with(info->env, &info->argv[i][1], '=');
		if (node)
		{
			rep_string(&(info->argv[i]),
				_strdup(_stringchr(node->str, '=') + 1));
			continue;
		}
		rep_string(&info->argv[i], _strdup(""));

	}
	return (0);
}

/**
 * rep_string - rep a string
 * @old: address of old string
 * @new: new str
 * Return: 1 or 0
 */
int rep_string(char **old, char *new)
{
	free(*old);
	*old = new;
	return (1);
}
